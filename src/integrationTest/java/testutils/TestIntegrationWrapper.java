...
public class TestIntegrationWrapper {
  ...
  void startAll() throws Exception {
    ...
    // --- Start Envoy ---
    // Relies on pre-built Envoy image with custom Dockerfile and entrypoint
    // Cannot build at runtime because package installation during build is flaky in test env
  ...
  private String getAdminToken(String baseUrl, String username, String password)
      throws IOException {
    RequestBody body =
  ...
  private String lookupUserId(String adminToken, String username) throws IOException {
    HttpUrl url =
        Objects.requireNonNull(HttpUrl.parse(keycloakBaseUrl + "/admin/realms/" + REALM + "/users"))
            .newBuilder()
  ...
  void missingToken_is401() throws Exception {
    Request req = new Request.Builder().url(envoyBaseUrl + "/__test/whoami").get().build();
